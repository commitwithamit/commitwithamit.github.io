...
if(!empty($cpf)){
    $get_email = mysqli_query($db_connect, "select email, dark_mode from registered_employee where cpf='$cpf'");
    if(mysqli_num_rows($get_email)>0){
        $email_fet = mysqli_fetch_array($get_email);
        $email = $email_fet["email"];
        $dark_mode = $email_fet["dark_mode"];
        if($dark_mode == "dark"){
            $dark = "dark";
            $d_block = "d-block";
        }
...
if($_POST){
    if(!empty($_POST["otp"])){
        ...
        $otp_len = strlen($otp);
        if($otp_len == 6){
            $otp_chk_query = mysqli_query($db_connect, "select * from registered_employee where otp='$otp' and cpf='$cpf'");
            if(mysqli_num_rows($otp_chk_query)>0){
                ...
                $update_otp_blank = mysqli_query($db_connect, "update registered_employee set otp='' where cpf='$cpf'");
                if($update_otp_blank == true){
                    unset($_SESSION["cpf_sess"]);   
                    # check_bio() will send user to homepage if bio exist in user_bio table and if not then user will be sent to fill bio before going to homepage
                    check_bio();
                }
            }else{
                $block = "d-block";
...
<body class="<?=$dark?>">
    ...
    <div class="main-con">
        ...
        <div class="box">
            ...
            <form action="otp.php?cpf=<?=$cpf?>" method="post" class="form-pd">
