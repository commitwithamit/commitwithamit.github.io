...
if(!empty($mod_user_id)){
    $get_email = mysqli_query($db_connect, "select email from registered_employee where user_id='$mod_user_id'");
...
if($_POST){
    if(!empty($_POST["otp"])){
        ...
        $otp_len = strlen($otp);
        if($otp_len == 6){
            $otp_chk_query = mysqli_query($db_connect, "select * from ongc_moderators where otp='$otp' and user_id='$mod_user_id'");
            if(mysqli_num_rows($otp_chk_query)>0){
                $fetch = mysqli_fetch_array($otp_chk_query);
                $mod_id =  $fetch["mod_id"];
                $update_otp_blank = mysqli_query($db_connect, "update ongc_moderators set otp='' where user_id='$mod_user_id'");
                if($update_otp_blank == true){
                    unset($_SESSION["mod_user"]); 
...
<body class="<?=$dark?>">
    ...
    <div class="main-con">
        ...
        <div class="box">
            ...
            <form action="moderator-otp.php" method="post" class="form-pd">
